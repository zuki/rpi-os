...
// outputs
int clear() {
    if (erase() == ERR) return ERR;
    if (refresh() == ERR) return ERR;
    return OK;
}

int erase() {
    int i, j;
    printf("\033[2J");  // clear screenbuf
    fflush(stdout);
    for (i = 0; i < LINES; ++i) {
        for (j = 0; j < COLS; ++j) {
...
int move(int y, int x) {
    if (y < 0 || LINES <= y) return ERR;
    if (x < 0 || COLS <= x) return ERR;
...
int mvaddch(int y, int x, char c) {
    ...
    if (addch(c) == ERR) return ERR;
    return OK;
}

int mvaddstr(int y, int x, char* str) {
    if (move(y, x) == ERR) return ERR;
    if (addstr(str) == ERR) return ERR;
    return OK;
}
